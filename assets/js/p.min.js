 ...
 console.log(answer)
  answer.forEach(item => {
    item.addEventListener('click',function(){
        let itemAnswer= item.textContent
        answer1.push(itemAnswer)
    })
  });
...
// const ncommentBlock = commentBlock.nextElementSibling
let templateComment = ``;
...
for (let i=0; i<nodes.length; i++){
    ...
    console.log(not)
    const comment = {}
    comment.img = not.querySelector('div > img')?.src
    comment.id =String (Math.random()).split('.')[1]
    comment.textContent = not.querySelector(' div.comment-content > p:nth-child(2) > font > font')?.innerText
    comment.countLike = not.querySelector(' div.comment-status > span > font:nth-child(3) > font')?.innerText
...
console.log(JSON.parse(JSON.stringify(comments,null,2)))
